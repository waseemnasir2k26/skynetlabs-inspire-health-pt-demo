...
// Distraction-free funnel pages: no global header/footer, one CTA (max attention
// ratio). Send paid/social lead-magnet traffic straight to #/guide.
const BARE = new Set(["guide"]);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Object.entries(BRAND.vars).forEach(([k, v]) =>
  ...
  const bare = BARE.has(page);

  return (
    <div className="grain min-h-screen">
      {!bare && <Header page={page} go={go} />}
      <main>
        <Page go={go} sub={sub} />
      </main>
      {!bare && <Footer go={go} />}
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5 sm:px-8">
        <button
          ...
          aria-label="Inspire Health PT — home"
        >
        ...
        <nav className="hidden items-center gap-5 xl:flex">
          {NAV.map(({ p, label }) => (
            <button
              key={p}
              onClick={() => go(p)}
              className={`ulink whitespace-nowrap text-sm tracking-wide ${page === p ? "text-accent" : "text-ink"}`}
            >
              {label}
            </button>
          ))}
      ...
      {open && (
        <div className="border-t border-line bg-bg px-5 py-4 xl:hidden">
          {NAV.map(({ p, label }) => (
            <button
              key={p}
              onClick={() => {
              ...
              className="block w-full py-2.5 text-left text-ink"
            >
              {label}
            </button>
          ))}
          <button
            ...
            className="btn-primary mt-3 w-full px-5 py-3"
          >
    ...
    <footer className="mt-24 border-t border-line bg-surface">
      <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <div className="grid gap-10 md:grid-cols-4">
          <div className="md:col-span-2">
            ...
            <p className="serif max-w-sm text-lg" style={{ color: "#cfcabb" }}>
              One room. One doctor. One hour. Private 1-on-1 rehab and
              performance in Miami — we don’t manage pain, we build you past it.
            </p>
            <button
              onClick={() => go("evaluation")}
          ...
          <div>
            ...
            {NAV.map(({ p, label }) => (
              <button
                key={p}
                onClick={() => go(p)}
                className="ulink mb-2.5 block text-sm text-muted"
              >
                {label}
              </button>
            ))}
          ...
          <div>
            ...
            {CLINIC.phone && (
              <a
                ...
                className="ulink mb-3 flex items-center gap-2 text-sm text-muted"
              >
            ...
            <a
              ...
              target="_blank"
              rel="noopener"
              className="ulink mb-2.5 block text-sm text-ink"
            >
              Book online ↗
            </a>
            <a
              href={CLINIC.freeCall}
              target="_blank"
              rel="noopener"
              className="ulink block text-sm text-muted"
            >
              Free 15-min call ↗
            </a>
          </div>
        ...
        <div className="space-y-3 text-xs text-muted2">
          ...
          <p className="pt-2">
            © {year} Inspire Health PT · The Vault. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
