...
import {
  Section, Reveal, Eyebrow, DiamondRule, Btn, VSL, CTABand, FAQList, Icon, Stars,
} from '../ui.jsx'
import { HERO, METHOD, PRICING, FAQ, TESTIMONIALS, DISCLAIMERS, CLINIC } from '../data.js'
...
function LeadForm({ go }) {
  ...
  return (
    <form
      onSubmit={(e) => { e.preventDefault(); setBusy(true); setTimeout(() => go('thank-you/eval'), 700) }}
      className="card p-6 sm:p-7"
    >
      <h3 className="display text-2xl">Prefer we reach out? Leave your details.</h3>
      <p className="serif mt-2 text-sm text-muted">We’ll confirm a time and send everything you need before your visit.</p>
      <div className="mt-5 space-y-3">
        ...
        />
        <textarea
          rows={3} value={msg} onChange={(e) => setMsg(e.target.value)} placeholder="What’s going on?"
          className="w-full rounded-theme border border-line bg-bg px-4 py-3 text-ink outline-none focus:border-accent"
        />
      </div>
      <button type="submit" disabled={busy} className="btn-ghost mt-5 w-full px-6 py-3.5 font-semibold disabled:opacity-60">
        {busy ? 'Sending…' : 'Request a callback'}
      </button>
      <p className="mt-3 text-center text-xs text-muted">Demo form · no message is sent</p>
    </form>
  )
}
  ...
  return (
    <main>
      ...
      <header
        ...
        style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(15,48,36,0.5), transparent 62%), linear-gradient(180deg,#11150F,var(--c-bg))' }}
      >
        <Section className="pt-28 pb-20 text-center sm:pt-36 sm:pb-24">
          ...
          <Reveal delay={60}><p className="eyebrow mt-6 mb-5">Book Your Evaluation</p></Reveal>
          <Reveal delay={120}>
            <h1 className="display mx-auto max-w-[18ch] text-5xl sm:text-7xl">
              A real diagnosis. A real plan. <span className="italic text-accent">In one hour.</span>
            </h1>
          </Reveal>
          <Reveal delay={180}>
            <p className="lede mx-auto mt-7 max-w-2xl">
              One private hour with a Doctor of Physical Therapy. We find why the pain is there, treat it the same day,
              and send you home with a plan you actually understand.
            </p>
          </Reveal>
          <Reveal delay={240}>
            <div className="mt-9 flex flex-wrap items-center justify-center gap-4">
              <Btn href={CLINIC.calendly} size="lg">Book Your Evaluation</Btn>
              <Btn href={CLINIC.freeCall} variant="underline">Free 15-min call</Btn>
            </div>
          </Reveal>
          <Reveal delay={300}>
            <div className="mt-7 flex items-center justify-center gap-3 text-sm text-muted">
              <Stars /> <span>{TESTIMONIALS.rating} · {TESTIMONIALS.count} reviews</span>
            </div>
          </Reveal>
        </Section>
      </header>
      ...
      <Section className="py-20">
        <Reveal className="text-center"><Eyebrow>Watch first</Eyebrow></Reveal>
        <Reveal delay={60}>
      ...
      <Section className="py-20">
        <Reveal className="text-center"><Eyebrow>The visit</Eyebrow></Reveal>
        <Reveal delay={60}>
          ...
          {VISIT_STEPS.map((s, i) => (
            <Reveal key={s.n} delay={i * 70}>
              <div className="card-brass h-full p-6">
                ...
                <p className="serif mt-2 leading-relaxed text-muted">{s.body}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <Section className="py-16">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Reveal delay={60}>
            <h2 className="display text-3xl sm:text-5xl">You’ll know if this is you.</h2>
          </Reveal>
        </div>
      ...
      {/* 5 — INLINE CALENDLY #1 -------------------------------------------- */}
      <Section className="py-20">
        <Reveal className="text-center"><Eyebrow>Reserve your time</Eyebrow></Reveal>
        <Reveal delay={60}>
          <h2 className="display mx-auto max-w-[16ch] text-center text-4xl sm:text-6xl">Pick your time.</h2>
        </Reveal>
        <Reveal delay={140}>
      ...
      <Section className="py-16">
        <div className="mx-auto max-w-3xl text-center">
          <Reveal><Eyebrow>{PRICING.eyebrow}</Eyebrow></Reveal>
          <Reveal delay={60}>
        ...
        <Reveal delay={120}>
          <div className="card-green mx-auto mt-10 max-w-xl p-8 sm:p-10">
            ...
            <Btn href={CLINIC.calendly} size="lg" className="mt-8 w-full text-center">{tier.cta}</Btn>
            <p className="mt-4 text-center text-xs text-muted">{PRICING.note}</p>
          </div>
        </Reveal>
        <Reveal delay={180}>
          <p className="mx-auto mt-6 max-w-xl text-center text-xs text-muted">{DISCLAIMERS.network}</p>
        </Reveal>
      </Section>
      ...
      <CTABand
        ...
        go={go}
        secondary={<Btn href={CLINIC.freeCall} variant="underline">Free 15-min call</Btn>}
      />
    </main>
  )
}
