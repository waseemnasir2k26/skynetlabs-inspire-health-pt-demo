import { Section, Reveal, DiamondRule, PageHero, CTABand, Btn } from '../ui.jsx'
...
function PlaceholderFrame({ caption, ratio = 'aspect-[4/5]', className = '' }) {
  return (
    <div className={`flex items-center justify-center bg-surface2 border border-line ${ratio} ${className}`}>
      <p className="serif px-6 text-center text-sm italic text-muted2">{caption}</p>
    </div>
  )
}
  ...
  return (
    <main>
      ...
      <Section className="py-24 text-center sm:py-28">
        ...
        <Reveal delay={80}><DiamondRule /></Reveal>
      </Section>
        ...
        <div className="space-y-28 sm:space-y-36">
          {amenities.map((a, i) => {
            ...
            const media = a.photo ? (
              ...
              <PlaceholderFrame caption={`Photography of ${a.name} — client image drops here`} />
            )
            ...
            return (
              <Reveal key={a.name}>
                <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
                  {/* media */}
                  <div className={flip ? 'lg:order-2' : 'lg:order-1'}>{media}</div>
                  {/* text */}
                  <div className={flip ? 'lg:order-1' : 'lg:order-2'}>
                    ...
                    {a.link === 'gallery' && (
                      <div className="mt-7">
                        <Btn to="gallery" go={go} variant="underline">See the gallery</Btn>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <Section className="py-24 sm:py-28">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse at 50% 0%, rgba(0,0,0,0.25), transparent 70%), var(--c-green)' }}
        >
          <div className="text-center">
            <Reveal>
              <p className="eyebrow mb-4" style={{ color: 'var(--c-accent-soft)' }}>{walkthrough.eyebrow}</p>
            </Reveal>
            <Reveal delay={80}>
              <h2 className="display mx-auto max-w-[18ch] text-4xl sm:text-5xl" style={{ color: '#fff' }}>
                {walkthrough.title}
              </h2>
            </Reveal>
          </div>
            ...
            {walkthrough.steps.map((s, i) => (
              <Reveal key={s.n} delay={i * 90}>
                <div>
                  ...
                  <h3 className="display mt-5 text-2xl" style={{ color: '#fff' }}>{s.title}</h3>
                  <p className="serif mt-3 text-lg leading-relaxed" style={{ color: '#dce8e2' }}>{s.body}</p>
                </div>
              </Reveal>
            ))}
          </div>
